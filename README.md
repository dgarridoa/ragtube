...
RAGTube is a Retrieval Augmented Generation (RAG) application that lets you query YouTube video transcriptions from your favorite YouTubers. It retrieves the most relevant transcription chunks for your query and provides them as context to a large language model (LLM) to generate responses.
...
• CLI and Database Tools: A Typer-based command line interface is provided for database population and managing the HNSW index creation. It facilitates the downloading of data, creation of transcriptions, dividing transcriptions into manageable chunks, computing the embeddings and create the HNSW index if does not exist.

• Containerization & Deployment: Docker Compose is used to orchestrate the entire application stack including the PostgreSQL database, Ollama, backend API, frontend UI, and additional services such as Watchtower for automated rolling releases and Traefik serving as a reverse proxy with SSL handling.
